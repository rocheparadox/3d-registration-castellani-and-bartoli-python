...
import numpy as np
from utils.utils import calculate_centroid, calculate_euclidean_distance, calculate_cross_covariance_matrix, \
...
def registration(model_view, data_view, show_plot=False, save_figure=True):
    ...
    while sse > 0.1 and iteration < max_iterations:  # iteration < 10 is a safeguard to prevent infinite loop. Should be removed later
        ...
        if save_figure or show_plot:
            plot_utils.plot_2d_model_and_data(model_view, data_view,
                                              ...
                                              dpi=300, title=f'iteration {iteration}', save_figure=True)
        ...
        translation_matrix = model_view_mean - np.matmul(rotation_matrix, data_view_mean)
